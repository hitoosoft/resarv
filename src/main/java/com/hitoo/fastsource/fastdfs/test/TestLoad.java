...
import com.hitoo.fastsource.fastdfs.DownloadCallback;
	...
	public static class Uploader {
		...
		public int uploadFile() throws Exception {
			...
			try {
				...
				System.out.println("upload file fail, error mesg: "
						+ ex.getMessage());
				return -1;
			}
		}
	...
	public static class Downloader {
		...
		public int downloadFile(String file_id) throws Exception {
			...
			try {
				...
				if (errno != 0) {
					System.out.println("Download file fail, file_id: "
							+ file_id + ", error no: " + errno);
				}
				return errno;
			} catch (Exception ex) {
				System.out.println("Download file fail, error mesg: "
						+ ex.getMessage());
				return -1;
			}
		}
	...
	public static class DownloadThread extends Thread {
		...
		public void run() {
			try {
				...
				for (int i = 0; i < 3
						&& TestLoad.total_download_count < TestLoad.total_upload_count; i++) {
					file_id = (String) TestLoad.file_ids.poll();
					if (file_id == null) {
						Thread.sleep(10);
						continue;
