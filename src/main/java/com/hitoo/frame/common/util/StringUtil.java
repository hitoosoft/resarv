...
public class StringUtil {
	...
	public static String getOracleSQLNotIn( String field,List<?> ids) {
		return getOracleSQLNotIn(field ,ids ,1000);
	}
	
    public static String getOracleSQLIn( String field,List<?> ids) {
		return getOracleSQLIn(field ,ids ,1000);
	}
	 ...
	 */
	private static String getOracleSQLIn(String field, List<?> ids, int count) {
	    ...
	    if(len==1){
	    	String value=ids.get(0)==null?"":(String)ids.get(0);
	    	return "("+field + " = '"+value+"'"+")";
	    }else{
	...
	private static String joinStringWithoutBlank(List<?> ids,String separator){
		if(ids==null||ids.size()<=0){
			...
			for (int i = 0; i < ids.size(); i++) {
				if(ids.get(i)!=null){
					if(ids.get(i) instanceof java.lang.String){
						if(StringUtils.isBlank((String)ids.get(i))){
							continue;
						}
					}
	...
	private static String getOracleSQLNotIn( String field,List<?> ids, int count) {
	    count = Math.min(count, 1000);
	    int len = ids.size();
	    int size = len % count;
	    if (size == 0) {
	    ...
	    for (int i = 0; i < size; i++) {
	        int fromIndex = i * count;
	        int toIndex = Math.min(fromIndex + count, len);
	        String productId = StringUtils.defaultIfEmpty(joinStringWithoutBlank(ids.subList(fromIndex, toIndex), "','"), "");
	    ...
	    String rtnStr=builder.toString();
