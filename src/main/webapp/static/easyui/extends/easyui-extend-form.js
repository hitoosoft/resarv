				...
				$form.find("input[name], textarea[name], input[numberboxname], select[name]").each(function() {
					...
					if ($input.hasClass("combo-value")) {
						...
						if ($input.hasClass("easyui-combobox")) {
							...
							$input.datebox("setValue", hitooctrl.hitooDateStringFormat(value));
						} else if ($input.hasClass("easyui-datetimebox")) {							
							$input.datetimebox("setValue",hitooctrl.hitooDateTimeStringFormat(value));
			...
			if ($form.attr('columns')) {
				...
				$inputs.each(function(index, input) {
					...
					if(syscode){
						...
						var dicArray = hitooctrl.dicValuesWithAll(syscode, rootflag);
						if(dicArray.length > 0){
							$input.combobox('loadData', dicArray);
						}else{
							$input.combobox('reload', hitooctrl.webprojectname+"/sys/dic/queryDicItemByCode.do?syscode="+syscode+"&rootflag="+rootflag);
						}
					}
				});
			}
	...
	function setFormContent(form){
		...
		$inputs.each(function(index, input) {
            ...
            if ($input.attr("type") == "checkbox" || $input.attr("type") == "radio") {       
            	...
            	if (label) {
            		//var $td = $("<td align='right' />").appendTo($tr).append($input);
            		if (colspan && ! vertical) {                			
    					colspan = parseInt(colspan);
    					$("<td align='left' colspan ='"+(colspan * 2 - 1)+"' ></td>").appendTo($tr).append($input).append( "&nbsp;&nbsp;" +label );
    					//$("<td align='left' colspan ='"+(colspan * 2 - 1)+"' >　" + label+"&nbsp;&nbsp;" + "</td>").appendTo($tr).append($input);
    					
    				}else{
    					$("<td align='left'></td>").appendTo($tr).append($input).append( "&nbsp;&nbsp;" +label );
    					//$("<td align='left'>　" + label+"&nbsp;&nbsp;"  + "</td>").appendTo($tr).append($input);
